...
def getImageArr1(img, width, height):
    img = cv.cvtColor(img, cv.COLOR_GRAY2RGB)
    img = np.float32(cv.resize(img, (width, height))) / 127.5 - 1
    return img


def getSegmentationArr1(img, nClasses, width, height):
...
def TransResUnet_Fcn(train_data, train_target, test_data, sol=None):
    ...
    model.fit(X_train, Y_train, validation_split=0.1, batch_size=32, epochs=sol[1],steps_per_epoch=5,callbacks=[earlystopper, checkpointer])
    pred_img = model.predict(X_test)
    ret_img = pred_img[:, :, :, 0]
    X = []
    Y = []
    Z = []
    for im, seg in zip(train_data, train_target):
        X.append(getImageArr1(im, 128, 128))
        Y.append(getSegmentationArr1(seg, 3, 128, 128))
        Z.append(getImageArr1(im, 128, 128))
    X, Y = np.array(X), np.array(Y)
    fcnn_Im = FCN(X, Y,Z,sol[1:])
    final_image = cv.bitwise_and(ret_img, fcnn_Im)
    return  final_image
